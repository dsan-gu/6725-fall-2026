...
All prose in this repo must follow the writing skill at
`.claude/skills/writing/SKILL.md`. This covers lecture slides, `docs/` pages, the
README, commit messages, and PR descriptions. Apply Orwell's six rules, prefer the
active voice, and cut the LLM tells: dead metaphors, -ly padding, and summary
paragraphs. Run the skill's revision pass before you ship any writing.

## Python code style

Applies to `build_site.py` and `scripts/`. The full standards are in the
"Detailed coding standards" section at the end of this file (AGENTS.md is
canonical; `CLAUDE.md` just points here). The essentials:
...
- `.claude/skills/writing/SKILL.md`: the writing skill all prose in this repo follows
- `CLAUDE.md`: a thin pointer back to this file; AGENTS.md is the canonical source
...
The sections below are the full coding standards for this repo. They used to
live in `CLAUDE.md`; they are canonical here now so there is one source of
truth. They apply to `build_site.py`, `scripts/`, and any Python or shell code
added to the repo.
