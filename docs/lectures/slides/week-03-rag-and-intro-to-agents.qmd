...
## How We Used to Fight Hallucinations {.smaller}

In the earliest days of GenAI, the main defense was to beg, right in the prompt. Every production prompt carried a line like one of these:

```text
Answer ONLY from the context provided. If the answer is not in the
context, say "I don't know."
```

```text
If you are not sure, respond with "I do not have enough information
to answer this question." Do NOT make up an answer.
```

```text
Never guess. If the documents below do not contain the answer,
reply exactly: I don't know.
```

That was the state of the art -- instructions were the only tool anyone had. Today's models are trained to decline when they lack grounding, so the begging does much less work. A line like this still helps at the margin, and it costs a dozen tokens -- but as you will see in the lab, it may no longer be mandatory.
...
- **Qdrant**: Open source, rich filtering and hybrid search; runs from laptop to production. **Our lab uses it embedded** -- a Python package writing to a local folder, no server.
...
- **Chroma**: Small, embeddable store; two lines of Python to first query.
...
### Hands-on Exercise -- same models as the Week 2 lab, everything local

- **No API keys, no cloud calls**: **nomic-embed-text** for embeddings and **MiniCPM5-2B** for generation, both through Ollama -- the same two models you already pulled
- Vectors live in **Qdrant**, a real vector database running as an embedded Python package: no server, no Docker, just a `qdrant_db/` folder next to your code
- Build the full pipeline: load, chunk, embed, index, retrieve, answer -- about a corpus no model could know from training
- Then **break it**: make the model hallucinate on purpose, watch it happen
- Then **fix it**: metadata filtering and query rewriting -- today's techniques, applied
